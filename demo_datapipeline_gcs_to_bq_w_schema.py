...
def merge_data(transaction_path,conversion_rate_path,output_path):
    # อ่านข้อมูลจากไฟล์ สังเกตว่าใช้ path จากที่รับ parameter มา
...
with DAG(
    "workshop5_gcs_to_bq_w_schema",
    stat_date=days_ago(1),
    ...
    t4 = GCSToBigQueryOperator(
        task_id='gcs_to_bq',
        bucket='gcs-cloud-bucket',
        source_object=['data/output.csv'],
        destination_project_dataset_table='dataset.tablename',
        # skip_leading_rows=1 เพื่อเป็นการ skip header เพราะถ้าใช้ headerจะผิดschema
        skip_leading_rows=1,
        # schema_fields ใช้ bq command run ใน terminal cloud shell "bq show --schema --format prettyjson [dataset].[table_name]"
        schema_fields=[
            {
                "mode": "NULLABLE",
                "name": "timestamp",
        ...
        # WRITE_TRUNCATE เป็นการให้สร้าง table ใหม่ทุกครั้งถ้าอยากให้เพิ่มเข้าไปใช้ WRITE_APPEND
